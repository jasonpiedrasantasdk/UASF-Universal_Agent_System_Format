...
in the UASF-Universal_Agent_System_Format repository (`Core/`). Match
its structure, depth, and discipline. When in doubt about format or
section order, the exemplar wins.
...
Filename convention: `<agent_slug>_uasf_v<N>_final.md`. If a working
directory is declared in the user's session preferences or prior turn,
write there. Otherwise propose the path before writing.
...
session, then write it to disk via available filesystem tools at the
declared path. If no filesystem tool is available, deliver as a code
...
v1.1 — packaged as a Claude Code subagent (agents/uasf-agent-architect.md)
alongside the existing slash command, so it can be auto-delegated to
rather than requiring manual invocation.
